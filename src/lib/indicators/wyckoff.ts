...
export function analyzeWyckoff(data: WyckoffCandle[], params: {
  ...
  for (const range of consolidations) {
    analyzeRange(data, vsa, range.startIdx, range.endIdx, range.low, range.high, events, zones, poes, invalidations, phaseRanges);
  }
...
// ─── Manual Zone Analysis: User selects a range, we analyze it directly ───
...
export function analyzeWyckoffZone(data: WyckoffCandle[], startIdx: number, endIdx: number, params: {
  ...
  // Find the range boundaries from the user-selected zone
  ...
  analyzeRange(data, vsa, startIdx, endIdx, rangeLow, rangeHigh, events, zones, poes, invalidations, phaseRanges);
...
function analyzeRange(
  ...
  const supportZone = rangeLow + rangeSize * 0.15;
  const resistanceZone = rangeHigh - rangeSize * 0.15;
  const lastDataIdx = Math.min(data.length - 1, endIdx + 50);
  ...
  for (let i = startIdx; i <= Math.min(startIdx + 15, endIdx); i++) {
    if (data[i].low < scLow) { scLow = data[i].low; scIdx = i; }
  }
  ...
  let arIdx = -1, arHigh = 0;
  for (let i = scIdx + 1; i <= Math.min(scIdx + 20, endIdx); i++) {
    if (data[i].high > arHigh) { arHigh = data[i].high; arIdx = i; }
  }
    ...
    for (let i = arIdx + 1; i <= Math.min(arIdx + 25, endIdx); i++) {
      ...
      if (nearSC && data[i].volume < data[scIdx].volume * 0.75) {
        ...
        events.push({
          ...
          price: data[i].low, phase: 'A', label: 'ST',
    ...
    if (data[i].low < rangeLow) {
      ...
      if (recoveredSameBar || recoveredNextBar) {
        ...
        events.push({
          ...
          price: data[i].low, phase: 'C', label: 'Spring',
  ...
  for (let i = phaseDStart; i <= lastDataIdx; i++) {
    ...
    if (sosIdx === -1 && data[i].close > resistanceZone && data[i].close > data[i].open && data[i].volume > vsa.volAvg[i]) {
      sosIdx = i;
      events.push({
    ...
    if (sosIdx !== -1 && buIdx === -1 && i > sosIdx + 1) {
      ...
      if (nearResistance && data[i].volume < vsa.volAvg[i] * 0.8) {
        ...
        events.push({
          ...
          price: data[i].low, phase: 'D', label: 'BU',
  ...
  if (springIdx !== -1) {
    for (let j = springIdx + 3; j < Math.min(data.length, springIdx + 30); j++) {
      if (data[j].close < rangeLow && data[j].volume > vsa.volPercentile90 && (data[j].high - data[j].low) > vsa.spreadAvg[j] * 1.2) {
        invalidations.push({
